...
class AudioCircularBuffer {
  ...
  public:
    typedef Ty value;
    AudioCircularBuffer(int initSize, bool newWay)
    ...
    bool dataAvailable(int requireLength) {
        return mAvailSamples >= requireLength;
    }
    static int IntModule(int ptrIndex, int frmLength, int bufLength)
    {
    ...
    int mAvailSamples = 0;
    int mReadPtrPosition = 0;
    int mWritePtrPosition = 0;
    int mInt16BufferLength;
    value* pInt16BufferPtr;
    AgoraExternalResampler::scoped_array<value> pInt16Buffer;
